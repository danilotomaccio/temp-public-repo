...
export const Scheduler: FC<SchedulerProps> = ({
  ...
  const [frequenza, setFrequenza] = useState<string>(defaultFrequency);
  ...
  const processedData = useMemo(() => {
    ...
    if (filters.terapistiPiuScarichi || filters.preferenzaTerapistaUnico) {
      ...
      if (filters.preferenzaTerapistaUnico && filteredByWorkload.length > 0) {
        ...
        if (therapistCounts[Number(bestTherapistId)] >= totalSessions) {
    ...
    if (availableGreenSlotsCount < totalSessions) {
      warningMessage = `Attenzione: Sono stati trovati solo ${availableGreenSlotsCount} slot che soddisfano tutti i filtri, ma ne servono ${totalSessions}. Prova a disattivare qualche filtro o esplora le opzioni colorate per trovare più disponibilità.`;
    }

    return { slots: finalSlots, warningMessage };
  }, [patientAvailability, filters, selectedTherapistId, totalSessions]);
        ...
        <div className="lg:col-span-1 bg-gray-50 p-6 rounded-lg border border-gray-200">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-md text-gray-900 font-semibold">{cycleName}</p>
            </div>
            <div>
              <label className="text-sm font-medium text-gray-600">
                N° Sedute Richieste
              </label>
              <p className="text-md text-gray-900">{totalSessions}</p>
            </div>
            <div>
              <label
                htmlFor="frequenza"
